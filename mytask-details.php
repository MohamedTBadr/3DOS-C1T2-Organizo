...
// Check if task_id and user_id are valid
if ( $task_id > 0 && $user_id > 0) {
    ...
    if (isset($_GET['deletee'])) {
      $id = $_GET['deletee'];
      $delete = "DELETE FROM `task_notes` WHERE `note_id` = '$id'";
      $run_delete = mysqli_query($connect, $delete);
      if (!$run_delete) {
          die('Delete query failed: ' . mysqli_error($connect));
      } else {
          header("location:mytask-details.php?task_id=$task_id");
      }
  }
} 
...
<?php
                            $cid = $task['task_id'];
                            $commentQuery = "SELECT note_id, first_name, last_name, note FROM `task_notes`
...
<i class="fa-solid fa-download orange"></i>
      ...
      <!-- Delete Comment -->
       <div class="me-2">
      <a href="mytask-details.php?task_id=<?php echo $task_id; ?>&deletee=<?php echo $fetch_comment['note_id']?>"><i class="fa-solid fa-trash orange" ></i></a>
    </div>
</div>
  </div>
